<div class="modal fade" id="delete-account">
		...
		<!-- My contact request -->
		<div class="modal fade" id="my-contact-request">
			<div class="modal-dialog modal-dialog-centered">
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title">
							<span class="material-icons group-add-btn">phone_add</span>My contacts request
						</h5>
						<button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
						<span class="material-icons">close</span>
						</button>
					...
					<div class="modal-body">
						<table class="table">
						     @php
							  $requestArr=\App\Models\ContactList::where('user_id_2',Auth::user()->id)->where('status',0)->where('type','send')->get();
							@endphp
							@if(isset($requestArr) && !empty($requestArr) && $requestArr->count()>0)
							  @foreach($requestArr as $contact_request)
								<tr>
									<th>
										<img style="width:100px;height:100px;" src="{{URL::asset('uploads/user/profile_pic/')}}/{{$contact_request->getContactRequest->profile_pic}}" alt="">
										{{$contact_request->getContactRequest->name}}
									</th>
									<td>
										<button    
									    onclick="getContactRequest({{$contact_request->getContactRequest->id}})"
										style="background-color: #420BA1;
										border-color: #420BA1;
										color: #fff;
										border-radius: 10px;" 
										class="btn btn-update mb-1 accept_{{$contact_request->getContactRequest->id}}">+ Accept
										</button>
									</td>
								</tr>
							   @endforeach
